...
# The class acts as a container for our questions and related logic.
class IELTSQuestionBank:
    """
    A class to manage and provide random IELTS speaking test questions.
    """
    def __init__(self):
        """
        Initializes the question bank. The question data is stored
        as an instance variable.
        """
        # We prefix with an underscore (_) to indicate that this list is
    ...
    def _validate_question_structure(self, question_set) -> bool:
        """
    ...
    def _validate_question_bank(self) -> bool:
        ...
        """
        try:
            if not isinstance(self._questions, list) or len(self._questions) == 0:
                logger.error("Question bank is not a valid non-empty list")
                return False
            
            for i, question_set in enumerate(self._questions):
                if not self._validate_question_structure(question_set):
                    logger.error(f"Invalid question set at index {i}")
                    return False
            
            logger.info(f"Question bank validation successful: {len(self._questions)} question sets")
            return True
    ...
    def get_random_test(self) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not self._validate_question_bank():
            ...
            selected_test = random.choice(self._questions)
...
if __name__ != "__main__":
    ...
    ielts_question_bank = IELTSQuestionBank()
    if not ielts_question_bank._validate_question_bank():
